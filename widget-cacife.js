...
/* PL: borda arredondada do modal */@media(min-width:768px){.q-card-ia,.q-card,#q-card-ia,#q-card,.q-modal-card{border-radius:16px !important;overflow:hidden;}}
    ...
    function init() {
        ...
        const CACIFE_STORE_WHATSAPP = '5511915390708';
        function showLimitWhatsApp() {
            var lb = document.getElementById('q-loading-box');
            var su = photoStep;
            ...
            if (link) {
                ...
                link.addEventListener('click', function () { try { fetch('https://n8n.segredosdodrop.com/webhook/pl-provador-limit-wa-click', { method: 'POST', keepalive: true, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ phone: (document.getElementById('q-phone') || document.querySelector('#q-modal-ia input[type=tel], input[type=tel]') || {}).value || '', origin: location.origin, produto: (prodName || '').trim() }) }).catch(function () {}); } catch (e) {} });
            }
        }
...
const fd = new FormData();
        ...
        genBtn.onclick = async () => {
            ...
            try {
                ...
                if (data.limited) {
                    // limite atingido: oferece prova extra por R$1 via PIX
                    // (a tela de PIX tem link discreto pra consultora pra quem nao quiser pagar)
                    try { document.getElementById('q-loading-box').style.display = 'none'; } catch (_) {}
...
(function () {
    ...
    script.src = 'https://lucasdecamargosilva.github.io/nuvemshopcacife/lentes-cacife-piloto.js?v=20260909-3';
    script.async = true;
